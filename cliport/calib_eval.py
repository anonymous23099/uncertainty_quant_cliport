...
@hydra.main(config_path="./cfg", config_name='calib_eval')
def main(cfg):
    # Logger
    ...
    if cfg['calibration']['enabled']:
        if cfg['calibration']['training']:
            ...
            test_ds = RavensDataset(os.path.join(data_dir, '{}-test'.format(task)), cfg, n_demos=n_test, augment=False)
            agent = agents.names[agent_type](name, cfg, test_ds, test_ds)
            for param in agent.attention.parameters():
                param.requires_grad = False
            for param in agent.transport.parameters():
                param.requires_grad = False
    ...
    if cfg['calibration']['enabled'] and not cfg['calibration']['training']:
        overall_test_results = []
        for _ in range(cfg['calibration']['n_test_steps']//cfg['calibration']['n_test']):
            test_results = trainer.test(agent)
            overall_test_results.extend(test_results)
        # import pdb; pdb.set_trace()
        with open(f"{cfg['train']['train_dir']}/test_results.pkl", 'wb') as f:
            pickle.dump(overall_test_results, f)
